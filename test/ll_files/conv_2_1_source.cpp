...
// Now outputting a linebuffer
 linebuffer_hls_stream_AxiPackedStencil_uint16_t_1_1___to_hls_stream_PackedStencil_uint16_t_1_2___bnds_64_64 _lb__hw_input_stencil_update_stream_to__hw_input_stencil_stream;
	for (int i = 0; i < 65; i++) {
...
// Steady state for loop to write and read (void)0;
 // dispatch_stream(_hw_input_stencil_stream, 2, 1, 1, 64, 2, 1, 64, 1, "conv", 0, 0, 64, 0, 64);
...
#pragma HLS RESOURCE variable=_hw_input_stencil_stream_to_conv core=FIFO_SRL
 ...
 for (int _dim_1 = 0; _dim_1 <= 62; _dim_1 += 1)
 for (int _dim_0 = 0; _dim_0 <= 63; _dim_0 += 1)
 {
#pragma HLS PIPELINE
  PackedStencil_uint16_t_1_2_ _tmp_stencil = _hw_input_stencil_stream.read();
  if (_dim_0 >= 0 && _dim_0 <= 63 && _dim_1 >= 0 && _dim_1 <= 62)
  {
...
#pragma HLS RESOURCE variable=_conv_stencil_stream core=FIFO_SRL
 ...
 for (int _conv_y___scan_dim_1 = 0; _conv_y___scan_dim_1 < 0 + 63; _conv_y___scan_dim_1++)
 {
  for (int _conv_x___scan_dim_0 = 0; _conv_x___scan_dim_0 < 0 + 64; _conv_x___scan_dim_0++)
  {
...
_conv_stencil_stream.write(_conv_stencil);
 ...
 // dispatch_stream(_conv_stencil_stream, 2, 1, 1, 64, 1, 1, 63, 1, "hw_output", 0, 0, 64, 0, 63);
...
// -- End of dispatch
 ...
 for (int _hw_output_y___scan_dim_1 = 0; _hw_output_y___scan_dim_1 < 0 + 63; _hw_output_y___scan_dim_1++)
 {
  for (int _hw_output_x___scan_dim_0 = 0; _hw_output_x___scan_dim_0 < 0 + 64; _hw_output_x___scan_dim_0++)
  {
#pragma HLS PIPELINE II=1
// realize stencil or stencil update conv.stencil
   Stencil_int32_t_1_1_ _conv_stencil;
...
AxiPackedStencil_uint16_t_1_1_ _hw_output_stencil_packed = _hw_output_stencil;
   if (_hw_output_x___scan_dim_0 == 63 && _hw_output_y___scan_dim_1 == 62) {
